...
如果希望在 RT 结果基础上运行 Sionna SYS 链路自适应，并获取服务基站以及链路/信道相关指标，请勾选 `Enable Sionna SYS link adaptation on top of RT`，然后填写参数，或直接使用默认值。此时会出现名为 `sys_observations` 的 rostopic。
...
点击 `Start Simulation`，仿真将会启动。你可以在 3D 视图窗口中查看仿真结果。
...
如果 rosbag 中包含 RF、SYS、beam 和 UAV 位姿 topic 中的任意一种，rosbag 中的 RF、SYS 或 beam 数据会自动播放。如果 rosbag 不包含这 3 类 topic，但包含 UAV 位姿 topic，则会播放 UAV 位姿 topic，此时可以点击 `Start Simulation`，基于 rosbag 中的 UAV 位置启动仿真。
