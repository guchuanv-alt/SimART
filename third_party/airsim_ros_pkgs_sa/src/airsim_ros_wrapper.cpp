...
namespace {
constexpr double kExternalCameraPublishLeaseTimeoutSec = 5.0;
}
...
AirsimROSWrapper::AirsimROSWrapper(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private, const std::string& host_ip)
    ...
    , imu_async_spinner_(1, &imu_timer_cb_queue_) // same as above, but for IMU (high-rate)
    ...
    , tf_listener_(tf_buffer_)
{
    ...
    // Fixed axis permutation used to convert AirSim camera "body" frame to ROS optical frame.
    // Equivalent to: x_opt = y_body, y_opt = z_body, z_opt = x_body.
    // Precomputing avoids per-frame Matrix3x3 work inside publish_camera_tf().
    tf2::Matrix3x3 P;
    P.setValue(0, 0, 1,
               1, 0, 0,
               0, 1, 0);
    P.getRotation(q_cam_body_to_optical_);
    q_cam_body_to_optical_.normalize();
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    origin_geo_point_pub_ = nh_private_.advertise<airsim_ros_pkgs_sa::GPSYaw>("origin_geo_point", 10);

    airsim_img_request_vehicle_name_pair_vec_.clear();
    optional_vehicle_image_requests_.clear();
    external_camera_controls_.clear();
    image_pub_vec_.clear();
    image_topic_vec_.clear();
    image_pub_last_request_wall_time_vec_.clear();
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
        ...
        vehicle_ros->env_pub = nh_private_.advertise<airsim_ros_pkgs_sa::Environment>(curr_vehicle_name + "/environment", 10);
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            drone->vel_cmd_body_frame_sub = nh_private_.subscribe<airsim_ros_pkgs_sa::VelCmd>(
            ...
            drone->vel_cmd_world_frame_sub = nh_private_.subscribe<airsim_ros_pkgs_sa::VelCmd>(
                curr_vehicle_name + "/vel_cmd_world_frame",
                1,
                boost::bind(&AirsimROSWrapper::vel_cmd_world_frame_cb, this, _1, vehicle_ros->vehicle_name));

            drone->takeoff_srvr = nh_private_.advertiseService<airsim_ros_pkgs_sa::Takeoff::Request, airsim_ros_pkgs_sa::Takeoff::Response>(
                curr_vehicle_name + "/takeoff",
                boost::bind(&AirsimROSWrapper::takeoff_srv_cb, this, _1, _2, vehicle_ros->vehicle_name));

            drone->land_srvr = nh_private_.advertiseService<airsim_ros_pkgs_sa::Land::Request, airsim_ros_pkgs_sa::Land::Response>(
            ...
            car->car_cmd_sub = nh_private_.subscribe<airsim_ros_pkgs_sa::CarControls>(
                curr_vehicle_name + "/car_cmd",
                1,
                boost::bind(&AirsimROSWrapper::car_cmd_cb, this, _1, vehicle_ros->vehicle_name));

            car->car_state_pub = nh_private_.advertise<airsim_ros_pkgs_sa::CarState>(curr_vehicle_name + "/car_state", 10);
        }
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras) {
            ...
            std::vector<int> current_pub_indices;
            ...
            for (const auto& curr_capture_elem : camera_setting.capture_settings) {
                ...
                if (!(std::isnan(capture_setting.fov_degrees))) {
                    ...
                    const bool pixels_as_float = !(curr_image_type == ImageType::Scene ||
                                                   curr_image_type == ImageType::Segmentation ||
                                                   curr_image_type == ImageType::SurfaceNormals ||
                                                   curr_image_type == ImageType::Infrared);
                    ImageRequest image_request(curr_camera_name, curr_image_type, pixels_as_float, false);
                    ...
                    const int pub_idx = static_cast<int>(image_pub_vec_.size());
                    if (curr_image_type == ImageType::Scene) {
                        ...
                        current_pub_indices.push_back(pub_idx);
                    }
                    else {
                        const ros::SubscriberStatusCallback status_cb =
                            boost::bind(&AirsimROSWrapper::image_subscriber_status_cb, this, _1, pub_idx);
                        image_pub_vec_.push_back(nh_private_.advertise<sensor_msgs::Image>(cam_image_topic, 1, status_cb, status_cb));
                        optional_vehicle_image_requests_.push_back({ image_request, pub_idx, curr_vehicle_name, curr_vehicle_name });
                    }
                    image_topic_vec_.push_back(nh_private_.resolveName(cam_image_topic));
                    image_pub_last_request_wall_time_vec_.push_back(ros::WallTime(0));
            ...
            // push back always-on RGB/Scene requests. Non-RGB vehicle requests are activated
            // by subscriber connect/disconnect callbacks so they do not touch the high-rate
            // RGB path while unused.
            if (!current_image_request_vec.empty()) {
                airsim_img_request_vehicle_name_pair_vec_.push_back({ current_image_request_vec, current_pub_indices, curr_vehicle_name, curr_vehicle_name, false, false });
            }
        }
        ...
        for (const auto& [sensor_name, sensor_setting] : vehicle_setting->sensors) {
            if (sensor_setting->enabled) {
                ...
                case SensorBase::SensorType::Barometer: {
                    ...
                    sensor_publisher.publisher = nh_private_.advertise<airsim_ros_pkgs_sa::Altimeter>(curr_vehicle_name + "/altimeter/" + sensor_name, 10);
                    break;
                }
        ...
        // Same idea for IMU: keep a dedicated list so we can poll/publish IMU on its own fast timer
        // without iterating other (usually slower) sensors.
        auto isImu = [](const SensorPublisher& pub) {
            return pub.sensor_type == SensorBase::SensorType::Imu;
        };
        const size_t imu_local_cnt = std::count_if(vehicle_ros->sensor_pubs.begin(), vehicle_ros->sensor_pubs.end(), isImu);
        imu_cnt += imu_local_cnt;
        if (imu_local_cnt > 0) {
            vehicle_ros->imu_pubs.resize(imu_local_cnt);
            std::vector<SensorPublisher> other_sensors;
            other_sensors.resize(vehicle_ros->sensor_pubs.size() - imu_local_cnt);
            std::partition_copy(vehicle_ros->sensor_pubs.begin(), vehicle_ros->sensor_pubs.end(),
                                vehicle_ros->imu_pubs.begin(), other_sensors.begin(), isImu);
            vehicle_ros->sensor_pubs = std::move(other_sensors);
        }
    ...
    for (const auto& curr_camera_elem : AirSimSettings::singleton().external_cameras) {
        const auto& curr_camera_name = curr_camera_elem.first;
        const auto& camera_setting = curr_camera_elem.second;
        if (!external_camera_ros_enabled(curr_camera_name)) {
            ROS_INFO_STREAM("External camera " << curr_camera_name << " ROS publishing control disabled by settings.");
            continue;
        }

        ExternalCameraPublisherControl external_control;
        external_control.camera_name = curr_camera_name;
        ...
        for (const auto& curr_capture_elem : camera_setting.capture_settings) {
            ...
            if (curr_image_type == ImageType::Scene ||
                ...
                curr_image_type == ImageType::SurfaceNormals ||
                curr_image_type == ImageType::Infrared) {
                external_control.requests.push_back(ImageRequest(curr_camera_name, curr_image_type, false, false));
            }
            else {
                external_control.requests.push_back(ImageRequest(curr_camera_name, curr_image_type, true, false));
            }

            const std::string cam_image_topic = external_camera_image_topic(curr_camera_name, capture_setting.image_type);
            external_control.image_topics.push_back(cam_image_topic);
        }

        if (!external_control.requests.empty()) {
            external_control.pub_indices.assign(external_control.requests.size(), -1);
            const size_t control_index = external_camera_controls_.size();
            external_camera_controls_.push_back(std::move(external_control));
            ExternalCameraPublisherControl& stored_control = external_camera_controls_.back();
            const std::string service_name = "external_cameras/" + curr_camera_name + "/set_publishing";
            stored_control.set_publishing_srvr =
                nh_private_.advertiseService<std_srvs::SetBool::Request, std_srvs::SetBool::Response>(
                    service_name,
                    boost::bind(&AirsimROSWrapper::set_external_camera_publishing_srv_cb, this, _1, _2, control_index));
            ROS_INFO_STREAM("External camera " << curr_camera_name
                            << " ROS image topics are disabled by default; enable with ~/"
                            << service_name << ".");
        }
    }
    ...
    {
        std::lock_guard<std::mutex> lock(image_request_mutex_);
        rebuild_optional_vehicle_image_request_pairs_locked();
    }

    // if >0 cameras, add one more thread for img_request_timer_cb
    if (!airsim_img_request_vehicle_name_pair_vec_.empty() || !optional_vehicle_image_requests_.empty() || !external_camera_controls_.empty()) {
        ...
        ros::WallTimerOptions timer_options(ros::WallDuration(update_airsim_img_response_every_n_sec),
                                            boost::bind(&AirsimROSWrapper::img_response_timer_cb, this, _1),
                                            &img_timer_cb_queue_);

        airsim_img_response_timer_ = nh_private_.createWallTimer(timer_options);
    ...
    // IMU can be polled/published on its own (potentially faster) timer so high IMU rates
    // don't get bottlenecked by the heavier vehicle state loop (env/odom/etc.).
    if (imu_cnt > 0) {
        double update_imu_every_n_sec;
        nh_private_.param("update_imu_every_n_sec", update_imu_every_n_sec, 0.01); // default 100 Hz

        ros::TimerOptions timer_options(ros::Duration(update_imu_every_n_sec),
                                        boost::bind(&AirsimROSWrapper::imu_timer_cb, this, _1),
                                        &imu_timer_cb_queue_);

        airsim_imu_update_timer_ = nh_private_.createTimer(timer_options);
...
bool AirsimROSWrapper::takeoff_srv_cb(airsim_ros_pkgs_sa::Takeoff::Request& request, airsim_ros_pkgs_sa::Takeoff::Response& response, const std::string& vehicle_name)
{
...
bool AirsimROSWrapper::takeoff_group_srv_cb(airsim_ros_pkgs_sa::TakeoffGroup::Request& request, airsim_ros_pkgs_sa::TakeoffGroup::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::takeoff_all_srv_cb(airsim_ros_pkgs_sa::Takeoff::Request& request, airsim_ros_pkgs_sa::Takeoff::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::land_srv_cb(airsim_ros_pkgs_sa::Land::Request& request, airsim_ros_pkgs_sa::Land::Response& response, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::land_group_srv_cb(airsim_ros_pkgs_sa::LandGroup::Request& request, airsim_ros_pkgs_sa::LandGroup::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::land_all_srv_cb(airsim_ros_pkgs_sa::Land::Request& request, airsim_ros_pkgs_sa::Land::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::reset_srv_cb(airsim_ros_pkgs_sa::Reset::Request& request, airsim_ros_pkgs_sa::Reset::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::car_cmd_cb(const airsim_ros_pkgs_sa::CarControls::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
// void AirsimROSWrapper::vel_cmd_body_frame_cb(const airsim_ros_pkgs_sa::VelCmd& msg, const std::string& vehicle_name)
void AirsimROSWrapper::vel_cmd_body_frame_cb(const airsim_ros_pkgs_sa::VelCmd::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_group_body_frame_cb(const airsim_ros_pkgs_sa::VelCmdGroup& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
// void AirsimROSWrapper::vel_cmd_all_body_frame_cb(const airsim_ros_pkgs_sa::VelCmd::ConstPtr& msg)
void AirsimROSWrapper::vel_cmd_all_body_frame_cb(const airsim_ros_pkgs_sa::VelCmd& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_world_frame_cb(const airsim_ros_pkgs_sa::VelCmd::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_group_world_frame_cb(const airsim_ros_pkgs_sa::VelCmdGroup& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_all_world_frame_cb(const airsim_ros_pkgs_sa::VelCmd& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::gimbal_angle_quat_cmd_cb(const airsim_ros_pkgs_sa::GimbalAngleQuatCmd& gimbal_angle_quat_cmd_msg)
{
...
void AirsimROSWrapper::gimbal_angle_euler_cmd_cb(const airsim_ros_pkgs_sa::GimbalAngleEulerCmd& gimbal_angle_euler_cmd_msg)
{
    try {
        tf2::Quaternion quat_control_cmd;
...
airsim_ros_pkgs_sa::CarState AirsimROSWrapper::get_roscarstate_msg_from_car_state(const msr::airlib::CarApiBase::CarState& car_state) const
{
    airsim_ros_pkgs_sa::CarState state_msg;
...
airsim_ros_pkgs_sa::Environment AirsimROSWrapper::get_environment_msg_from_airsim(const msr::airlib::Environment::State& env_data) const
{
...
airsim_ros_pkgs_sa::Altimeter AirsimROSWrapper::get_altimeter_msg_from_airsim(const msr::airlib::BarometerBase::Output& alt_data) const
{
    airsim_ros_pkgs_sa::Altimeter alt_msg;
...
airsim_ros_pkgs_sa::GPSYaw AirsimROSWrapper::get_gps_msg_from_airsim_geo_point(const msr::airlib::GeoPoint& geo_point) const
{
    airsim_ros_pkgs_sa::GPSYaw gps_msg;
...
ros::Time AirsimROSWrapper::update_state()
{
    ...
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            airsim_ros_pkgs_sa::CarState state_msg = get_roscarstate_msg_from_car_state(car->curr_car_state);
        ...
        airsim_ros_pkgs_sa::Environment env_msg = get_environment_msg_from_airsim(env_data);
    ...
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        for (auto& sensor_publisher : vehicle_ros->sensor_pubs) {
            ...
            case SensorBase::SensorType::Barometer: {
                ...
                airsim_ros_pkgs_sa::Altimeter alt_msg = get_altimeter_msg_from_airsim(baro_data);
                alt_msg.header.frame_id = vehicle_ros->vehicle_name;
                sensor_publisher.publisher.publish(alt_msg);
                break;
            }
            case SensorBase::SensorType::Imu: {
                // If IMU is handled by the dedicated high-rate IMU timer, skip here to
                // keep the main state loop lightweight.
                if (is_used_imu_timer_cb_queue_) {
                    break;
                }
...
void AirsimROSWrapper::img_response_timer_cb(const ros::WallTimerEvent& event)
{
    (void)event;
    try {
        std::vector<AirsimImgRequestVehicleNamePair> request_pairs;
        {
            std::lock_guard<std::mutex> lock(image_request_mutex_);
            expire_external_camera_publish_leases_locked();
            request_pairs = airsim_img_request_vehicle_name_pair_vec_;
        }

        for (const auto& airsim_img_request_vehicle_name_pair : request_pairs) {

            // Vehicle cameras keep RGB/Scene streams alive for normal consumers, but expensive
            // non-RGB captures (depth, segmentation, normals, infrared) are requested only when
            // their image topic is actively subscribed. External cameras intentionally keep the
            // existing on-demand publish-service behavior.
            std::vector<ImageRequest> filtered_requests;
            std::vector<int> filtered_pub_indices;
            filtered_requests.reserve(airsim_img_request_vehicle_name_pair.requests.size());
            filtered_pub_indices.reserve(airsim_img_request_vehicle_name_pair.pub_indices.size());

            {
                std::lock_guard<std::mutex> lock(image_request_mutex_);
                for (size_t i = 0; i < airsim_img_request_vehicle_name_pair.requests.size(); ++i) {
                    const int pub_idx = airsim_img_request_vehicle_name_pair.pub_indices.at(i);
                    if (pub_idx < 0 || pub_idx >= static_cast<int>(image_pub_vec_.size())
                        || pub_idx >= static_cast<int>(cam_info_pub_vec_.size())
                        || pub_idx >= static_cast<int>(image_topic_vec_.size())
                        || pub_idx >= static_cast<int>(image_pub_last_request_wall_time_vec_.size())) {
                        continue;
                    }
                    const ImageRequest& image_request = airsim_img_request_vehicle_name_pair.requests.at(i);
                    const bool is_scene_image = image_request.image_type == ImageType::Scene;
                    bool should_request = false;
                    if (!airsim_img_request_vehicle_name_pair.external) {
                        should_request = is_scene_image || airsim_img_request_vehicle_name_pair.optional;
                    }
                    else {
                        const bool image_has_subscribers = image_pub_vec_.at(pub_idx).getNumSubscribers() > 0;
                        const bool camera_info_has_subscribers = cam_info_pub_vec_.at(pub_idx) &&
                            cam_info_pub_vec_.at(pub_idx).getNumSubscribers() > 0;
                        should_request = image_has_subscribers || (is_scene_image && camera_info_has_subscribers);
                    }

                    if (should_request && image_topic_rate_limit_allows_request(pub_idx)) {
                        filtered_requests.push_back(image_request);
                        filtered_pub_indices.push_back(pub_idx);
                    }
                }
            }
            ...
            #ifdef AIRSIM_ROS_WRAPPER_DEBUG
            ros::Time simGetImages_start_time = ros::Time::now();
            #endif

            // IMPORTANT: store by value (move) so we can move pixel buffers later (avoid extra copies).
            std::vector<ImageResponse> img_response = airsim_client_images_.simGetImages(filtered_requests,
                                                                                        airsim_img_request_vehicle_name_pair.vehicle_name,
                                                                                        airsim_img_request_vehicle_name_pair.external);

            #ifdef AIRSIM_ROS_WRAPPER_DEBUG
            ros::Duration simGetImages_duration = ros::Time::now() - simGetImages_start_time;
            ROS_INFO_STREAM("simGetImages duration: " << simGetImages_duration.toSec() << " s.");
            #endif

            #ifdef AIRSIM_ROS_WRAPPER_DEBUG
            ros::Time process_and_publish_start_time = ros::Time::now();
            #endif

            if (img_response.size() == filtered_requests.size()) {
                std::lock_guard<std::mutex> lock(image_request_mutex_);
                process_and_publish_img_response(img_response, filtered_pub_indices, airsim_img_request_vehicle_name_pair.frame_id);
            }

            #ifdef AIRSIM_ROS_WRAPPER_DEBUG
            ros::Duration process_and_publish_duration = ros::Time::now() - process_and_publish_start_time;
            ROS_INFO_STREAM("process_and_publish_img_response duration: " << process_and_publish_duration.toSec() << " s.");
            #endif
        }
    }
...
void AirsimROSWrapper::imu_timer_cb(const ros::TimerEvent& event)
{
    try {
        for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
            auto& vehicle_ros = vehicle_name_ptr_pair.second;
            if (!vehicle_ros->imu_pubs.empty()) {
                for (auto& imu_publisher : vehicle_ros->imu_pubs) {
                    auto imu_data = airsim_client_imu_.getImuData(imu_publisher.sensor_name, vehicle_ros->vehicle_name);
                    sensor_msgs::Imu imu_msg = get_imu_msg_from_airsim(imu_data);
                    imu_msg.header.frame_id = vehicle_ros->vehicle_name;
        ...
        std::string msg = e.get_error().as<std::string>();
        std::cout << "Exception raised by the API, didn't get IMU data." << std::endl
                  << msg << std::endl;
    }
}
...
sensor_msgs::ImagePtr AirsimROSWrapper::get_img_msg_from_response(ImageResponse& img_response,
                                                                  const ros::Time curr_ros_time,
                                                                  const std::string frame_id)
{
    sensor_msgs::ImagePtr img_msg_ptr = boost::make_shared<sensor_msgs::Image>();
    img_msg_ptr->data = std::move(img_response.image_data_uint8);
    ...
    #ifdef AIRSIM_ROS_WRAPPER_DEBUG
    ROS_INFO("Image msg encoding: %s", img_msg_ptr->encoding.c_str());
    #endif
...
bool AirsimROSWrapper::boolish_setting(const msr::airlib::Settings& settings, const std::string& key, bool default_value) const
{
    ...
    try {
        return settings.getInt(key, default_value ? 1 : 0) != 0;
    }
    catch (...) {
    }
    try {
        std::string value = settings.getString(key, default_value ? "true" : "false");
...
bool AirsimROSWrapper::external_camera_ros_enabled(const std::string& camera_name) const
{
    msr::airlib::Settings external_cameras;
    if (!msr::airlib::Settings::singleton().getChild("ExternalCameras", external_cameras)) {
        return false;
    }

    msr::airlib::Settings camera_settings;
    if (!external_cameras.getChild(camera_name, camera_settings)) {
        return false;
    }

    if (camera_settings.hasKey("PublishToRos")) {
        return boolish_setting(camera_settings, "PublishToRos", false);
    }
    if (camera_settings.hasKey("PublishToROS")) {
        return boolish_setting(camera_settings, "PublishToROS", false);
    }
    if (camera_settings.hasKey("RosPublish")) {
        return boolish_setting(camera_settings, "RosPublish", false);
    }
    if (camera_settings.hasKey("ROSPublish")) {
        return boolish_setting(camera_settings, "ROSPublish", false);
    }
...
std::string AirsimROSWrapper::external_camera_image_topic(const std::string& camera_name, int image_type) const
{
    std::string configured_topic;
    msr::airlib::Settings external_cameras;
    if (msr::airlib::Settings::singleton().getChild("ExternalCameras", external_cameras)) {
        msr::airlib::Settings camera_settings;
        if (external_cameras.getChild(camera_name, camera_settings) && camera_settings.hasKey("RosTopic")) {
            try {
                configured_topic = camera_settings.getString("RosTopic", "");
            }
            catch (...) {
                configured_topic.clear();
            }
        }
    }

    const std::string image_type_name = image_type_int_to_string_map_.count(image_type) > 0
        ? image_type_int_to_string_map_.at(image_type)
        : std::to_string(image_type);

    if (!configured_topic.empty()) {
        const std::string placeholder = "{image_type}";
        const std::string::size_type pos = configured_topic.find(placeholder);
        if (pos != std::string::npos) {
            configured_topic.replace(pos, placeholder.size(), image_type_name);
            return configured_topic;
        }
        if (image_type == static_cast<int>(ImageType::Scene)) {
            return configured_topic;
        }
        return configured_topic + "/" + image_type_name;
    }

    const std::string suffix = "_camera";
    if (image_type == static_cast<int>(ImageType::Scene)
        && camera_name.size() > suffix.size()
        && camera_name.compare(camera_name.size() - suffix.size(), suffix.size(), suffix) == 0) {
        const std::string station_token = camera_name.substr(0, camera_name.size() - suffix.size());
        return "/airsim_gui/station_cameras/" + station_token + "/image_raw";
    }

    return "external_cameras/" + camera_name + "/" + image_type_name;
}

void AirsimROSWrapper::image_subscriber_status_cb(const ros::SingleSubscriberPublisher& publisher, int pub_idx)
{
    (void)publisher;
    std::lock_guard<std::mutex> lock(image_request_mutex_);
    if (pub_idx >= 0 && pub_idx < static_cast<int>(image_pub_last_request_wall_time_vec_.size())) {
        image_pub_last_request_wall_time_vec_[pub_idx] = ros::WallTime(0);
    }
    rebuild_optional_vehicle_image_request_pairs_locked();
}

void AirsimROSWrapper::rebuild_optional_vehicle_image_request_pairs_locked()
{
    airsim_img_request_vehicle_name_pair_vec_.erase(
        std::remove_if(airsim_img_request_vehicle_name_pair_vec_.begin(),
                       airsim_img_request_vehicle_name_pair_vec_.end(),
                       [](const AirsimImgRequestVehicleNamePair& pair) {
                           return pair.optional;
                       }),
        airsim_img_request_vehicle_name_pair_vec_.end());

    std::vector<AirsimImgRequestVehicleNamePair> active_optional_pairs;
    for (const OptionalVehicleImageRequest& optional_request : optional_vehicle_image_requests_) {
        const int pub_idx = optional_request.pub_idx;
        if (pub_idx < 0 || pub_idx >= static_cast<int>(image_pub_vec_.size())) {
            continue;
        }
        if (image_pub_vec_.at(pub_idx).getNumSubscribers() <= 0) {
            continue;
        }

        auto pair_it = std::find_if(active_optional_pairs.begin(), active_optional_pairs.end(),
                                    [&optional_request](const AirsimImgRequestVehicleNamePair& pair) {
                                        return pair.vehicle_name == optional_request.vehicle_name
                                            && pair.frame_id == optional_request.frame_id
                                            && !pair.external
                                            && pair.optional;
                                    });
        if (pair_it == active_optional_pairs.end()) {
            active_optional_pairs.push_back({ {}, {}, optional_request.vehicle_name, optional_request.frame_id, false, true });
            pair_it = std::prev(active_optional_pairs.end());
        }
        pair_it->requests.push_back(optional_request.request);
        pair_it->pub_indices.push_back(pub_idx);
    }

    airsim_img_request_vehicle_name_pair_vec_.insert(airsim_img_request_vehicle_name_pair_vec_.end(),
                                                     active_optional_pairs.begin(),
                                                     active_optional_pairs.end());
}

std::string AirsimROSWrapper::image_topic_fps_param_name(const std::string& topic_name) const
{
    std::ostringstream key;
    key << "image_topic_fps/t";
    for (const unsigned char c : topic_name) {
        if (std::isalnum(c)) {
            key << static_cast<char>(c);
        }
        else {
            key << '_' << std::hex << std::setw(2) << std::setfill('0')
...
bool AirsimROSWrapper::image_topic_rate_limit_allows_request(int pub_idx)
{
    if (pub_idx < 0 || pub_idx >= static_cast<int>(image_topic_vec_.size())
        || pub_idx >= static_cast<int>(image_pub_last_request_wall_time_vec_.size())) {
        return false;
    }

    double fps_limit = 0.0;
    nh_private_.param(image_topic_fps_param_name(image_topic_vec_[pub_idx]), fps_limit, 0.0);
    if (fps_limit <= 0.0) {
        return true;
    }

    fps_limit = std::max(1.0, std::min(100.0, fps_limit));
    const ros::WallTime now = ros::WallTime::now();
    const ros::WallTime last = image_pub_last_request_wall_time_vec_[pub_idx];
    if (!last.isZero() && (now - last).toSec() + 1e-4 < (1.0 / fps_limit)) {
        return false;
    }

    image_pub_last_request_wall_time_vec_[pub_idx] = now;
    return true;
}

void AirsimROSWrapper::disable_external_camera_publishing_locked(ExternalCameraPublisherControl& control)
{
    for (const int pub_idx : control.pub_indices) {
        if (pub_idx < 0 || pub_idx >= static_cast<int>(image_pub_vec_.size())) {
            continue;
        }
        image_pub_vec_[pub_idx].shutdown();
        if (pub_idx < static_cast<int>(image_pub_last_request_wall_time_vec_.size())) {
            image_pub_last_request_wall_time_vec_[pub_idx] = ros::WallTime(0);
        }
        if (pub_idx < static_cast<int>(cam_info_pub_vec_.size()) && cam_info_pub_vec_[pub_idx]) {
            cam_info_pub_vec_[pub_idx].shutdown();
        }
    }
    std::fill(control.pub_indices.begin(), control.pub_indices.end(), -1);
    control.enabled = false;
}

void AirsimROSWrapper::expire_external_camera_publish_leases_locked()
{
    const ros::WallTime now = ros::WallTime::now();
    bool changed = false;
    for (ExternalCameraPublisherControl& control : external_camera_controls_) {
        if (!control.enabled) {
            continue;
        }
        if ((now - control.last_enable_request_wall_time).toSec() <= kExternalCameraPublishLeaseTimeoutSec) {
            continue;
        }
        ROS_INFO_STREAM("External camera " << control.camera_name
                        << " ROS image topics disabled because the GUI publish lease expired.");
        disable_external_camera_publishing_locked(control);
        changed = true;
    }
    if (changed) {
        rebuild_external_camera_request_pair();
    }
}

bool AirsimROSWrapper::set_external_camera_publishing_srv_cb(std_srvs::SetBool::Request& request,
                                                             std_srvs::SetBool::Response& response,
                                                             size_t external_camera_index)
{
    std::lock_guard<std::mutex> lock(image_request_mutex_);
    if (external_camera_index >= external_camera_controls_.size()) {
        response.success = false;
        response.message = "Unknown external camera control index.";
        return true;
    }

    ExternalCameraPublisherControl& control = external_camera_controls_[external_camera_index];
    if (request.data == control.enabled) {
        if (request.data) {
            control.last_enable_request_wall_time = ros::WallTime::now();
        }
        response.success = true;
        response.message = control.enabled ? "External camera publishing is already enabled."
                                           : "External camera publishing is already disabled.";
        return true;
    }

    if (request.data) {
        control.pub_indices.assign(control.requests.size(), -1);
        for (size_t i = 0; i < control.requests.size(); ++i) {
            image_pub_vec_.push_back(nh_private_.advertise<sensor_msgs::Image>(control.image_topics.at(i), 1));
            image_topic_vec_.push_back(nh_private_.resolveName(control.image_topics.at(i)));
            image_pub_last_request_wall_time_vec_.push_back(ros::WallTime(0));
            cam_info_pub_vec_.push_back(ros::Publisher());
            control.pub_indices[i] = static_cast<int>(image_pub_vec_.size()) - 1;
            ROS_INFO_STREAM("External camera " << control.camera_name
                            << " advertises " << control.image_topics.at(i));
        }
        control.enabled = true;
        control.last_enable_request_wall_time = ros::WallTime::now();
        rebuild_external_camera_request_pair();
        response.success = true;
        response.message = "External camera publishing enabled.";
        return true;
    }

    disable_external_camera_publishing_locked(control);
    rebuild_external_camera_request_pair();
    ROS_INFO_STREAM("External camera " << control.camera_name << " ROS image topics disabled.");
    response.success = true;
    response.message = "External camera publishing disabled.";
    return true;
}

void AirsimROSWrapper::rebuild_external_camera_request_pair()
{
    airsim_img_request_vehicle_name_pair_vec_.erase(
        std::remove_if(airsim_img_request_vehicle_name_pair_vec_.begin(),
                       airsim_img_request_vehicle_name_pair_vec_.end(),
    ...
    std::vector<ImageRequest> external_image_request_vec;
    std::vector<int> external_pub_indices;
    for (const ExternalCameraPublisherControl& control : external_camera_controls_) {
        if (!control.enabled) {
            continue;
        }
        for (size_t i = 0; i < control.requests.size(); ++i) {
            const int pub_idx = control.pub_indices.at(i);
            if (pub_idx < 0) {
                continue;
            }
            external_image_request_vec.push_back(control.requests.at(i));
            external_pub_indices.push_back(pub_idx);
        }
    }

    if (!external_image_request_vec.empty()) {
        airsim_img_request_vehicle_name_pair_vec_.push_back({
            external_image_request_vec,
            external_pub_indices,
            std::string(),
            world_frame_id_,
            true
        });
...
void AirsimROSWrapper::process_and_publish_img_response(std::vector<ImageResponse>& img_response_vec,
                                                                   const std::vector<int>& pub_indices,
                                                                   const std::string& frame_id)
{
    // todo add option to use airsim time (image_response.TTimePoint) like Gazebo /use_sim_time param
    ros::Time curr_ros_time = ros::Time::now();

    if (img_response_vec.size() != pub_indices.size()) {
        ROS_WARN_STREAM("Image response size (" << img_response_vec.size()
                        << ") does not match pub index size (" << pub_indices.size()
                        << "). Skipping publish for frame " << frame_id);
        return;
    }

    for (size_t i = 0; i < img_response_vec.size(); ++i) {
        ImageResponse& curr_img_response = img_response_vec[i];
        const int pub_idx = pub_indices[i];
        if (pub_idx < 0 || pub_idx >= static_cast<int>(image_pub_vec_.size())
            || pub_idx >= static_cast<int>(cam_info_pub_vec_.size())) {
            continue;
        }
        ...
        publish_camera_tf(curr_img_response, curr_ros_time, frame_id, curr_img_response.camera_name);

        // CameraInfo publication is optional; skipping it when nobody is subscribed saves
        // one serialization + publish call per frame.
        if (cam_info_pub_vec_[pub_idx] && cam_info_pub_vec_[pub_idx].getNumSubscribers() > 0) {
            camera_info_msg_vec_[pub_idx].header.stamp = airsim_timestamp_to_ros(curr_img_response.time_stamp);
            cam_info_pub_vec_[pub_idx].publish(camera_info_msg_vec_[pub_idx]);
        }

        // DepthPlanar / DepthPerspective / DepthVis / DisparityNormalized
        if (curr_img_response.pixels_as_float) {
            image_pub_vec_[pub_idx].publish(get_depth_img_msg_from_response(curr_img_response,
                                                                           curr_ros_time,
                                                                           curr_img_response.camera_name + "_optical"));
        }
        // Scene / Segmentation / SurfaceNormals / Infrared
        else {
            image_pub_vec_[pub_idx].publish(get_img_msg_from_response(curr_img_response,
                                                                      curr_ros_time,
                                                                      curr_img_response.camera_name + "_optical"));
        }
    }
...
void AirsimROSWrapper::publish_camera_tf(const ImageResponse& img_response, const ros::Time& ros_time, const std::string& frame_id, const std::string& child_frame_id)
{
    ...
    // Avoid per-frame Matrix3x3 creation; just post-multiply by a precomputed constant rotation.
    tf2::Quaternion quat_cam_optical = quat_cam_body * q_cam_body_to_optical_;
    quat_cam_optical.normalize();
